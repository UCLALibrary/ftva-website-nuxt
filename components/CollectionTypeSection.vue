...
type SectionHandle =
  | 'ftvaListingMotionPictureCollections'
  | 'ftvaListingTelevisionCollections'
  | 'ftvaListingWatchAndListenOnline'
...
const { data, error } = await useAsyncData(props.canonicalPath, async () => {
  ...
  const data = await $graphql.default.request(FTVACollectionTypeListing, { section: props.sectionHandle })
  return data
...
const collectionType = ref(props.collection)
