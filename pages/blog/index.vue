...
const parsedArticles = computed(() => {
  ...
  return currentList.value.map((obj) => {
    return {
      ...
      image: { ...parseImage(obj), sizes: '(min-width: 1360px) 566px, calc(45.52vw - 44px)' },
...
const parsedFeaturedArticles = computed(() => {
  ...
  return featuredArticles.value.map((obj, index) => {
    ...
    // const sizesFeaturedArticles = index === 0 ? '' : '(min-width: 1360px) 1160px, (min-width: 760px) calc(91.03vw - 60px), (min-width: 420px) calc(100vw - 48px), calc(9vw + 316px)'
    // TODO need to have larger sizes for first image in featured articles
    return {
