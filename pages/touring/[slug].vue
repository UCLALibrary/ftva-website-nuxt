...
const parsedFtvaTouringSeries = computed(() => {
  /* Early Returns: If series.value is falsy or empty, or
    if firstSeries.ftvaTouringSeries is falsy or empty, return
    an empty array immediately. */
  if (!series.value || series.value.length === 0) {
    return []
  }

  const [firstTouringSeries] = series.value

  if (!firstTouringSeries.ftvaTouringSeries || firstTouringSeries.ftvaTouringSeries.length === 0) {
    return []
  }
