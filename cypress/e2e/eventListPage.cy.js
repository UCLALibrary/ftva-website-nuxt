...
describe('Events Listing page', () => {
  ...
  it('Toggles tab to calendar view & interacts with event', () => {
    ...
    cy.get('.calendar-event-item').first().as('firstEvent')
    cy.get('@firstEvent').click()
    cy.get('.calendar-event-item').should('be.visible')
    cy.get('.calendar-event-item').first().click()
    cy.get('.calendar-event-popup-wrapper').should('be.visible')
